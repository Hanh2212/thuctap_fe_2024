        ...
        <el-table :data="tableData" class="table_content">
            <el-table-column label="Hình ảnh" align="center" prop="anhDaiDien">
                <template #default="scope">
                    ...
                    /> </template
            ></el-table-column>
            <el-table-column label="Tên hãng" align="center" prop="tenHang">
                <template #default="scope">
            ...
            <el-table-column align="right">
                ...
                <template #default="scope">
                    ...
                    <el-popconfirm
                        ...
                        @confirm="() => confirmEvent(scope.row.maNhaSanXuat)"
                    >
                        <template #reference>
...
import { Manufactor } from "~/constant/api";
import {
    deleteManufactor,
    searchManufactor,
...
const tableData = ref<Manufactor[]>([]);
...
const handleEdit = (index: number, row: Manufactor) => {
    router.push(`/manufactor/edit/${row.maNhaSanXuat}`);
};
...
const fetchData = async (searchTerm: string = "") => {
    try {
        const res = await searchManufactor({
            page: currentPage.value,
            pageSize: 10,
            TenHang: searchTerm,
        });
        totalItemPage.value = res.totalItems;
        tableData.value = res.data.map(function (value: Manufactor) {
            return {
