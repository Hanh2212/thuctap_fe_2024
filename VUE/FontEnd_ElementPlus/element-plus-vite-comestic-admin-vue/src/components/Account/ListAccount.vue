            ...
            <el-table-column align="right">
                ...
                <template #default="scope">
                    ...
                    <el-popconfirm
                        ...
                        @confirm="() => confirmEvent(scope.row.maTaiKhoan)"
                    >
                        <template #reference>
...
import { Account } from "~/constant/api";
...
const totalItemPage = ref<number>(0);
...
const handleEdit = (index: number, row: Account) => {
    router.push(`/account/edit/${row.maTaiKhoan}`);
};
...
const fetchData = async (searchTerm: string = "") => {
    try {
        const res = await searchAccount({
            ...
            pageSize: 10,
            TenTaiKhoan: searchTerm,
        });
        totalItemPage.value = res.totalItems;
        tableData.value = res.data.map(function (value: Account) {
            return {
